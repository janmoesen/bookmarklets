	...
	*/; }).toString()
	...
	(function execute(document) {
		...
		function findContentElement() {
			...
			/* Choose the header whose text appears nearest the beginning of the page title. */
			if (headersInPageTitle.length) {
				headersInPageTitle.sort(function (a, b) {
					return a.substringIndex - b.substringIndex;
				});

				if (window.console) {
					console.log('Readable++: found suitable header element(s) whose text appears in the page title:');
					headersInPageTitle.map(function (headerInPageTitle) {
						console.log(headerInPageTitle.element, 'at substring index', headerInPageTitle.substringIndex);
					});
				}

				return headersInPageTitle[0].element;
			}
