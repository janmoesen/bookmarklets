...
(function sath() {
	...
	*/
	...
	if (s === '') {
		s = getActiveSelection() || prompt('Please enter the Strava athlete/member name to look for:');
	} else {
		s = s.replace(/(^|\s|")~("|\s|$)/g, '$1' + getActiveSelection() + '$2');
	}

	if (s) {
		location = 'https://www.strava.com/athletes/search?utf8=%E2%9C%93&text=' + encodeURIComponent(s);
	}
})();
