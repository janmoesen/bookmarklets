...
(function () {
	...
	if (s === '') {
		...
		if (!s) {
			...
			var interLanguageSelectors = [
				...
				'[class*="lang"][class*="elect"] a[hreflang^="es-"]',
				'a.language[href*="/es/"]',
				'a[class*="choose"][class*="lang"][href^="/es/"]',
			...
			var interLanguageXPathSelectors = [
				'//a[@href][translate(., "ABCÇDEFGHIJKLMNÑOPQRSTUVWXYZРУСКИЙ", "abcçdefghijklmnñopqrstuvwxyzруский") = "es"]',
				'//a[@href][translate(., "ABCÇDEFGHIJKLMNÑOPQRSTUVWXYZРУСКИЙ", "abcçdefghijklmnñopqrstuvwxyzруский") = "español"]',
			...
			if (possibleWikimediaDomainMatches) {
				...
				if (wikimediaDomains.indexOf(possibleWikimediaDomain) > -1 && !document.querySelector('.interlanguage-link')) {
					...
					/* ↓ NOTE ↓: `es` gets replaced when generating `2nl`, `2fr`, `2it`, …
					 * For Chinese, it becomes `zh-CN`, hence the suffix stripping. */
					const languageSubdomain = 'es'.replace(/-.*/, '');
					...
					console.log(`Translate to Spanish: Wikimedia special case: going to the corresponding page on the Spanish domain ${targetLanguageDomain}: ${urlForOtherLanguage}`);
			...
			if (!s) {
				s = prompt('Please enter your text to translate to Spanish:');
			}
		}
	} else {
	...
	if (s) {
		if (s.match(/^(https?:\/\/)([^\s.]+\.)+[^\s.]+\.?(\/\S*)?$/)) {
			...
			googleTranslateUrl.searchParams.set('_x_tr_tl', 'es');

			location = googleTranslateUrl;
		} else {
			location = 'https://translate.google.com/?op=translate&sl=auto&tl=es&text=' + encodeURIComponent(s);
		}
	}
})();
