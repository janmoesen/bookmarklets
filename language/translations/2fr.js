	...
	if (s === '') {
		...
		if (!s) {
			...
			var interLanguageSelectors = [
				...
				'[class*="lang"][class*="elect"] a[hreflang^="fr-"]',
				'a.language[href*="/fr/"]',
				'a[class*="choose"][class*="lang"][href^="/fr/"]',
			...
			if (possibleWikimediaDomainMatches) {
				...
				if (wikimediaDomains.indexOf(possibleWikimediaDomain) > -1 && !document.querySelector('.interlanguage-link')) {
					...
					/* ↓ NOTE ↓: `fr` gets replaced when generating `2nl`, `2fr`, `2it`, …
					 * For Chinese, it becomes `zh-CN`, hence the suffix stripping. */
					const languageSubdomain = 'fr'.replace(/-.*/, '');
					...
					console.log(`Translate to French: Wikimedia special case: going to the corresponding page on the French domain ${targetLanguageDomain}: ${urlForOtherLanguage}`);
