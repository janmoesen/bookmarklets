	...
	if (s === '') {
		...
		if (!s) {
			...
			var interLanguageXPathSelectors = [
				'//a[@href][translate(., "ABCÇDEFGHIJKLMNÑOPQRSTUVWXYZРУСКИЙ", "abcçdefghijklmnñopqrstuvwxyzруский") = "zh-CN"]',
				'//a[@href][translate(., "ABCÇDEFGHIJKLMNÑOPQRSTUVWXYZРУСКИЙ", "abcçdefghijklmnñopqrstuvwxyzруский") = "中文"]',
			...
			if (possibleWikimediaDomainMatches) {
				...
				if (wikimediaDomains.indexOf(possibleWikimediaDomain) > -1 && !document.querySelector('.interlanguage-link')) {
					...
					/* ↓ NOTE ↓: `zh-CN` gets replaced when generating `2nl`, `2fr`, `2it`, …
					...
					console.log(`Translate to Mandarin Chinese: Wikimedia special case: going to the corresponding page on the Mandarin Chinese domain ${targetLanguageDomain}: ${urlForOtherLanguage}`);
