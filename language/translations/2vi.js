		...
		if (!s) {
			...
			if (possibleWikimediaDomainMatches) {
				...
				if (wikimediaDomains.indexOf(possibleWikimediaDomain) > -1 && !document.querySelector('.interlanguage-link')) {
					...
					/* ↓ NOTE ↓: `vi` gets replaced when generating `2nl`, `2fr`, `2it`, …
					 * For Chinese, it becomes `zh-CN`, hence the suffix stripping. */
					const languageSubdomain = 'vi'.replace(/-.*/, '');
					...
					console.log(`Translate to Vietnamese: Wikimedia special case: going to the corresponding page on the Vietnamese domain ${targetLanguageDomain}: ${urlForOtherLanguage}`);
