...
(function () {
	...
	if (s === '') {
		...
		if (!s) {
			...
			var interLanguageSelectors = [
				...
				'[class*="lang"][class*="elect"] a[hreflang^="it-"]',
				'a.language[href*="/it/"]',
				'a[class*="choose"][class*="lang"][href^="/it/"]',
			...
			var interLanguageXPathSelectors = [
				'//a[@href][translate(., "ABCÇDEFGHIJKLMNÑOPQRSTUVWXYZРУСКИЙ", "abcçdefghijklmnñopqrstuvwxyzруский") = "it"]',
				'//a[@href][translate(., "ABCÇDEFGHIJKLMNÑOPQRSTUVWXYZРУСКИЙ", "abcçdefghijklmnñopqrstuvwxyzруский") = "italiano"]',
			...
			if (possibleWikimediaDomainMatches) {
				...
				if (wikimediaDomains.indexOf(possibleWikimediaDomain) > -1 && !document.querySelector('.interlanguage-link')) {
					...
					/* ↓ NOTE ↓: `it` gets replaced when generating `2nl`, `2fr`, `2it`, …
					 * For Chinese, it becomes `zh-CN`, hence the suffix stripping. */
					const languageSubdomain = 'it'.replace(/-.*/, '');
					...
					console.log(`Translate to Italian: Wikimedia special case: going to the corresponding page on the Italian domain ${targetLanguageDomain}: ${urlForOtherLanguage}`);
	...
	if (s) {
		if (s.match(/^(https?:\/\/)([^\s.]+\.)+[^\s.]+\.?(\/\S*)?$/)) {
			...
			googleTranslateUrl.searchParams.set('_x_tr_tl', 'it');

			location = googleTranslateUrl;
		} else {
			location = 'https://translate.google.com/?op=translate&sl=auto&tl=it&text=' + encodeURIComponent(s);
		}
	}
})();
