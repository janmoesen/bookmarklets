...
(function viwikt() {
	...
	*/
	...
	if (s === '') {
		s = getActiveSelection() || prompt('Please enter the word(s) to look up in the Vietnamese Wiktionary:');
	} else {
		s = s.replace(/(^|\s|")~("|\s|$)/g, '$1' + getActiveSelection() + '$2');
	}

	if (s) {
		location = 'https://vi.wiktionary.org/wiki/' + encodeURIComponent(s);
	}
})();
