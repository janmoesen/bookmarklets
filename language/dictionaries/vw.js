...
(function vw() {
	...
	*/
	...
	if (s === '') {
		s = getActiveSelection() || prompt('Please enter the word(s) to look up in the “Flemish” dictionary:');
	} else {
		s = s.replace(/(^|\s|")~("|\s|$)/g, '$1' + getActiveSelection() + '$2');
	}

	if (s) {
		location = 'http://www.vlaamswoordenboek.be/definities/zoek?definition[word]=' + encodeURIComponent(s);
	}
})();
