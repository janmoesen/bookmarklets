...
(function wikiv() {
	...
	*/
	...
	if (s === '') {
		s = getActiveSelection() || prompt('Please enter the subject to look up in the English Wikivoyage:');
	} else {
		s = s.replace(/(^|\s|")~("|\s|$)/g, '$1' + getActiveSelection() + '$2');
	}

	if (s) {
		/* The Wikivoyage search works like "I'm feeling lucky" on most Wikivoyage instances. If there is a complete match, it will redirect us there. */
		location = 'https://en.wikivoyage.org/w/index.php?searchToken=&title=Special%3ASearch&search=' + encodeURIComponent(s);
	}
})();
