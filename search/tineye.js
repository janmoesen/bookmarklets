...
(function tin() {
	...
	*/
	...
	if (s === '') {
		s = getActiveSelection() || prompt('Please enter the image URL for TinEye to reverse search:');
	} else {
	...
	if (s) {
		if (!s.match(/^(\w+:(\/\/)?)?[^\s]+(\.[^\s]+)+\//))
		{
			alert(`Are you sure that’s an image URL?\n\n${s}`);
			return;
		}

		location = 'https://tineye.com/search/?url=' + encodeURIComponent(s);
	}
})();
