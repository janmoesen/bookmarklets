...
(function ddimg(document) {
	...
	if (s === '') {
		s = getActiveSelection(document) || prompt('Please enter your DuckDuckGo image search query:');
	} else {
		s = s.replace(/(^|\s|")~("|\s|$)/g, '$1' + getActiveSelection(document) + '$2');
	}

	if (s) {
		location = 'https://duckduckgo.com/?ia=images&iax=images&q=' + encodeURIComponent(s);
	}
})(document);
