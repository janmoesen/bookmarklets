...
(function eimg(document) {
	...
	if (s === '') {
		s = getActiveSelection(document) || prompt('Please enter your Ecosia image search query:');
	} else {
		s = s.replace(/(^|\s|")~("|\s|$)/g, '$1' + getActiveSelection(document) + '$2');
	}

	if (s) {
		location = 'https://www.ecosia.org/images?q=' + encodeURIComponent(s);
	}
})(document);
