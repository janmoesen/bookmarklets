 ...
 *
 * @title Twitter user (Tweep) search
 */
(function tweep() {
	...
	*/
	...
	if (s === '') {
		s = getActiveSelection() || prompt('Please enter the Twitter user to search for:');
	} else {
		s = s.replace(/(^|\s|")~("|\s|$)/g, '$1' + getActiveSelection() + '$2');
	}

	if (s) {
		location = 'https://twitter.com/search?f=user&q=' + encodeURIComponent(s);
	}
})();
